...
#include "KrAFT/GenericNtuple/interface/GenericEventMET.h"
...
class KGenericNtupleMakerMET : public edm::EDAnalyzer
{
public:
  KGenericNtupleMakerMET(const edm::ParameterSet& pset);
  ~KGenericNtupleMakerMET();
...
private:
  ...
  edm::InputTag metMVALabel_;
  edm::InputTag metGENLabel_;
  ...
  // Output tree
  TTree* tree_;
  GenericEventMET* fevent_;

};

KGenericNtupleMakerMET::KGenericNtupleMakerMET(const edm::ParameterSet& pset)
{
  ...
  metMVALabel_ = jetMETPSet.getParameter<edm::InputTag>("metMVA");
  metGENLabel_ = jetMETPSet.getParameter<edm::InputTag>("metGEN");
  ...
  fevent_ = new GenericEventMET(isMC_);
  fevent_->book(tree_);
}

KGenericNtupleMakerMET::~KGenericNtupleMakerMET()
{
}

void KGenericNtupleMakerMET::endLuminosityBlock(const edm::LuminosityBlock& lumi, const edm::EventSetup& eventSetup)
{
  for ( int i=0, n=eventCounterLabels_.size(); i<n; ++i )
  {
...
void KGenericNtupleMakerMET::analyze(const edm::Event& event, const edm::EventSetup& eventSetup)
{
  using namespace std;
...
//    std::copy(pdfWeightsHandle->begin(), pdfWeightsHandle->end(), std::back_inserter(*fevent_->pdfWeights_));
  ...
  edm::Handle<std::vector<reco::PFMET>> metMVAHandle;
  event.getByLabel(metMVALabel_, metMVAHandle);  
  fevent_->metMVA_pt_ = metMVAHandle->at(0).pt();
  fevent_->metMVA_sumet_ = metMVAHandle->at(0).sumEt();
  fevent_->metMVA_phi_ = metMVAHandle->at(0).phi();
  ...
  fevent_->met_sumet_ = metHandle->at(0).sumEt();
  ...
  if ( isMC_ )
  {
    edm::Handle<std::vector<reco::GenMET>> metGENHandle;
    event.getByLabel(metGENLabel_, metGENHandle);
    fevent_->metGEN_pt_ = metGENHandle->at(0).pt();
    fevent_->metGEN_sumet_ = metGENHandle->at(0).sumEt();
    fevent_->metGEN_phi_ = metGENHandle->at(0).phi();
  ...
  if ( isMC_ )
  {
    ...
    fevent_->pdf_id1_ = 1;//pdf->id.first ;
    fevent_->pdf_id2_ = 1;//pdf->id.second;
    fevent_->pdf_x1_  = 1;//pdf->x.first  ;
    fevent_->pdf_x2_  = 1;//pdf->x.second ;
    fevent_->pdf_q_   = 1;//pdf->scalePDF ;
    //const double pdf_xPDF1 = pdf->xPDF.first, pdf_xPDF2 = pdf->xPDF.second;

    fevent_->genWeight_ = 1;//genEventInfoHandle->weights().at(0);
...
int KGenericNtupleMakerMET::findMother(const reco::GenParticle* p, std::vector<const reco::GenParticle*>& genParticles)
{
  if ( !p ) return -1;
...
DEFINE_FWK_MODULE(KGenericNtupleMakerMET);
